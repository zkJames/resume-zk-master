...
\end{tikzpicture}
  ...
  \textbf{国际EBK商品直连系统} 
  部门建立独立的海外EBK直连系统，负责Java后端开发工作，涉及\textbf{SpringBoot、MyBatis、Redis、MySQL、MetaQ（RocketMQ）}等技术栈。
  ...
  \begin{itemize}
    ...
    \item 了解\textbf{供应商数据中心MOS}需求。MOS为国际EBK提供经营能力，帮助供应商感知N漏斗全过程提升整体酒店/房型匹配率。
  \end{itemize}
  ...
  主要负责\textbf{后端系统}开发。涉及\textbf{Scope SQL、微软数据平台Cosmos、离线数仓、ABTest分析、Redis、线程池、Azure数据流}等技术和方法。
  \begin{itemize}
    ...
    \item 采用\textbf{代理模式}设计\textbf{代理层}和\textbf{业务层}。代理层负责与\textbf{搜索引擎和LLM模型服务接口进行交互}，实现搜索、模型的调用和结果返回。业务层负责处理用户请求和响应，实现写作工具的核心功能。为了提高框架的性能和生成质量，代理层采用了\textbf{接口响应优化、HTTP连接池、线程池、Redis缓存}等技术，有效地提升了并发请求的吞吐量和响应速度。
    \item Prompt工程：结合\textbf{推理}动作和\textbf{搜索}动作功能使ChatGPT模型可以与Bing搜索引擎交互，获取\textbf{实时热搜话题}和\textbf{Bing搜索结果}作为知识库，并生成更合理的类人任务解决轨迹，提高LLM新闻创作的真实性和创造性。
  \end{itemize}
  ...
  \role{数据平台后端开发实习生}{bilibili 数据平台部 Berserker}
  ...
  \begin{itemize}
    ...
    % ，优化了Hive表分区信息查询速度和资源占用，\textbf{接口性能提升60\%}
    \item 在部门降本增效计划中，实现无效数据表定期下线的功能，为业务提供打标，批量定时删除并在删除后及时告知表所有人的功能，通过\textbf{分布式定时调度框架}设计实现了高效的数据表安全删除逻辑功能
    \item 提供表使用热度的统计功能，参与了覆盖全、数据准、粒度细的\textbf{表使用热度}功能服务，为各类数据应用和数据治理提供支持。
  \end{itemize}
...
% 总结起来，通过对数据库层面的数据统计、分析和封装成服务接口，实现数据热度的收集和展示，这有助于提升大数据平台的数据应用价值，优化数据治理等业务操作，从而进一步提高数据质量和增强数据资产价值。
  ...
  \datedsubsection{\textbf{电力市场数据实时分析平台}}{2022年9月 - 至今} 
  \role{实验室项目}{数据工程、后端开发} 
  通过用电情况和电网公司内部的电力系统数据、日志数据、业务数据，建立实时数据仓库，以供电力公司管理人员及时了解用电时的运作情况和用电负荷。为用户提供与电力市场业务相关的报表、业务预测，以辅助各管理层有效决策。
  
  项目架构：Hadoop + Flink + Kafka + MySQL + HBase + Redis + ClickHouse + SpringBoot
    \begin{itemize} 
      \item 采用FlinkCDC2.0对MySQL中业务数据进行采集，从ODS层读取数据利用MySQL + 广播流实现动态分流。 
      \item 将数据分别写入Kafka、HBase构建DWD层和DIM层。
      \item 通过Flink将DWD层的数据进行实时聚合、过滤、分析，形成各种指标数据，并写入ClickHouse和Redis。ClickHouse作为OLAP引擎，支持多维度的实时查询和分析，Redis作为缓存数据库，提供快速的数据访问和更新。使用时间序列预测算法实现实时电价预测。
      \item 使用SpringBoot实现可视化大屏系统后端系统搭建。 
    \end{itemize} 

  \datedsubsection{\textbf{用于列车售票的可线性化并发数据结构}}{2021年9月 - 2021年12月} 
  \role{并发编程项目模块}{Java并发编程} 
 设计并实现了一个\textbf{可线性化的高效列车售票系统}，并能在并发环境下保证高性能运行。
    \begin{itemize} 
      \item 利用乘车区间\textbf{二进制编码}的涉及，将锁的粒度从区间级别降低到车座粒度，大大减少了锁的竞争和开销。 基于\textbf{CAS原语}（比较并交换），实现了\textbf{lock-free}（无锁）的并发方案，避免了死锁、饥饿等问题，提高了并发效率。 
