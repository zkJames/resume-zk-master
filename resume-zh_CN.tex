...
\basicInfo{
  ...
  \begin{itemize}
    ...
    \item 梳理商品信息抓取链路，从供应商接口获取酒店信息，分析酒店信息的报文结构，实现希尔顿酒店图片的批量保存和房型描述信息的解析和存储。
    \item 目前负责国际EBK直连系统的部分模块开发，主要负责用户账号鉴权、营销策略等模块的开发，运用\textbf{SpringBoot、Mybatis、Tair（缓存）、MetaQ（消息队列）、Switch（开关）、HSF（RPC）}等技术栈。
    \end{spacing} 
  \end{itemize}
